...
most_shortened_table = newtbl.where('Added length', max(newtbl.column('Added length'))).sort('Word', descending = True)
...
tbl_with_distance = train_movies.select('Title', 'Genre', 'money', 'feel').with_column('distance from batman', train_movies.apply(distance_from_batman_returns, 'Title')).sort('distance from batman')
...
bottom_left = 1 #or all questions below are 5 bc plot talks about average, not individual movies.
...
#my_20_features = make_array('start', 'new', 'hear','rememb', 'show', 'same', 'walk', 'next', 'few', 'understand', 'anoth', 'car', 'afraid', 'troubl', 'came', 'best', 'pay', 'street', 'women', 'buy')
#my_20_features = make_array('captain', 'ship', 'cop','head', 'hour', 'power', 'job', 'weve', 'gotta', 'mani', 'mom', 'mother', 'hello', 'wife', 'happi', 'school', 'huh', 'ladi', 'beauti', 'marri')
my_20_features = make_array('marri', 'captain', 'power', 'run', 'world', 'move', 'three', 'nice', 'boy', 'wouldnt', 
                            'enough', 'miss', 'head', 'cours', 'done', 'turn', 'yourself', 'home', 'job', 'knew')
...
# I looked for words that were furthest from the trend line to make the distinction clear on whether they belong in an action or romance film, and I focused on words toward the top right so that the occurrence was high enough for the word to have occurred more than once, making it a better predictor.
...
my_assigned_genre = genre_and_distances.select('Genre').take(np.arange(5)).group('Genre').sort('count', descending = True).column('Genre').item(0)
...
def classify(test_row, train_rows, train_labels, k):
    ...
    classified = genre_and_distances.select('Genre').group('Genre').sort('count', descending = True).column('Genre').item(0)
    return classified
